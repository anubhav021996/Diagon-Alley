...
export const Product = () => {
  ...
  useEffect(() => {
    setLoading(true);
    let api =
      cat.name == "Goblet of Products"
        ? `${process.env.REACT_APP_BASE_URL}/product?page=${page}&size=16`
        : `${process.env.REACT_APP_BASE_URL}/product/category/${cat.name}?page=${page}&size=16`;
    ...
    <>
      ...
      >
        <Stack direction={"row"} justifyContent="space-between" width={"100%"}>
          <Badge p={2} fontSize={"l"} cursor="pointer">
            {cat.name}
          </Badge>
