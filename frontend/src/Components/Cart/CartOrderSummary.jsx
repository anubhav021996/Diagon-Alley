...
export const CartOrderSummary = ({
  ...
  const handleCheckout = () => {
    for (let i = 0; i < items.length; i++) {
      if (items[i].count > items[i].quantity)
        return toast({
  ...
  const handlePayment = async () => {
    ...
    const data = await axios
      .post(`${process.env.REACT_APP_BASE_URL}/orders/razorpay`, {
        amount: Math.round(total + total * 0.18 - total * 0.1),
      })
      .then((res) => res.data);
    close();
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        Navigate("/paymentSuccess", {
          state: { id: response.razorpay_order_id },
        });
      },
      theme: {
        color: "#3399cc",
    ...
    <Stack spacing="8" borderWidth="1px" rounded="lg" padding="8" width="full">
      ...
      <Stack spacing="6">
        <OrderSummaryItem label="Subtotal">
          <Skeleton isLoaded={!editCart}>{formatPrice(total, "INR")}</Skeleton>
        </OrderSummaryItem>
        <OrderSummaryItem label="Tax">
          <Skeleton isLoaded={!editCart}>
            {formatPrice(total * 0.18, "INR")}
          </Skeleton>
        </OrderSummaryItem>
        <OrderSummaryItem label="Discount">
          <Skeleton isLoaded={!editCart}>
            -{formatPrice(total * 0.1, "INR")}
          </Skeleton>
        </OrderSummaryItem>
        ...
        <OrderSummaryItem label="Round-off">
          <Skeleton isLoaded={!editCart}>
            {formatPrice(
              total +
                total * 0.18 -
                total * 0.1 -
                Math.round(total + total * 0.18 - total * 0.1),
              "INR"
            )}
          </Skeleton>
        </OrderSummaryItem>
      ...
      {checkout ? (
        ...
        <Button
          ...
          size="lg"
          fontSize="md"
          rightIcon={!address && <FaArrowRight />}
          disabled={!items.length || address || editCart}
