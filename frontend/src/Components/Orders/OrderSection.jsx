...
export const OrderSection = ({ order }) => {
  ...
  return (
    <Box p={5} boxShadow="rgba(0, 0, 0, 0.35) 0px 5px 15px">
      ...
      <Text color={"gray.600"} fontSize={"l"} textAlign="left" mt={5}>
        <b>Payment Amount:</b> {formatPrice(order.amount, "INR")}
      </Text>
      <Text color={"gray.600"} fontSize={"l"} textAlign="left">
      ...
      <Text color={"gray.600"} fontSize={"l"} textAlign="left" mt={5}>
        ...
        {order.address?.substring(0, order.address.length - 20)}
      </Text>
    </Box>
  );
};
